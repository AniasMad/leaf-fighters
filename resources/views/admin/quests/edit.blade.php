...
<form enctype="multipart/form-data" action="{{ route('admin.quests.update', $quest->id) }}" method="post">
    ...
    @csrf
    @method('PUT')
    <img width="150" class="mb-4" src="{{ asset("storage/images/" . $quest->image) }}" alt="Current Image">
    <div class="mb-6">
        <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('Quest Title') }}</label>
        <input type="text" name="title" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ old('title') ? : $quest->title }}" required>
        @if($errors->has('title'))
    ...
    <div class="mb-6">
        ...
        <textarea id="description" name="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>{{ old('description', $quest->description) }}</textarea>
